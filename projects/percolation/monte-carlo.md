...
where M denotes the number of Monte Carlo samples, and the result can only be trusted within any statistical errors given by the standard error in the mean
...
Another basic property we can calculate is the mean cluster size $S$. This measures the typical size of the cluster to which a randomly chosen occupied site belongs (excluding the spanning cluster) and is defined by

$$
S = \frac{\sum_{s\neq\infty}s^2n_{s}}{\sum_{s\neq\infty}sn_{s}},
$$

where $n_{s}$ is the number of finite clusters of size $s$. We can implement the mean cluster size with the following function 
