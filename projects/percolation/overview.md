...
### Order Parameter: Percolation Strength $P_{\infty}$
...
where we exclude the infinite cluster when present ($p > p_{c}$). The $s^2$ weighting reflects that when randomly selecting an occupied site, larger clusters are proportionally more likely to be chosen.
